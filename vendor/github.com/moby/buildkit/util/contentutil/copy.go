...
type CopyInfo struct {
	Referrers func(context.Context, ocispecs.Descriptor) ([]ocispecs.Descriptor, error)
}

type CopyOption func(*CopyInfo) error

func WithReferrers(referrers func(context.Context, ocispecs.Descriptor) ([]ocispecs.Descriptor, error)) CopyOption {
...
func copyChain(ctx context.Context, ingester content.Ingester, provider content.Provider, desc ocispecs.Descriptor, visited *sync.Map, opts ...CopyOption) error {
	...
	for i := len(manifestStack) - 1; i >= 0; i-- {
		...
		if ci.Referrers != nil {
			referrers, err := ci.Referrers(ctx, desc)
			if err != nil {
