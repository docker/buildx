...
func New(root string) (*Store, error) {
	if err := osutil.MkdirAll(filepath.Join(root, instanceDir), 0700); err != nil {
		return nil, err
	}
	if err := osutil.MkdirAll(filepath.Join(root, defaultsDir), 0700); err != nil {
		return nil, err
	}
	if err := osutil.MkdirAll(filepath.Join(root, activityDir), 0700); err != nil {
		return nil, err
	}
...
func (t *Txn) Save(ng *NodeGroup) error {
	...
	return osutil.AtomicWriteFile(filepath.Join(t.s.root, instanceDir, name), dt, 0600)
}
...
func (t *Txn) SetCurrent(key, name string, global, def bool) error {
	...
	if err := osutil.AtomicWriteFile(filepath.Join(t.s.root, "current"), dt, 0600); err != nil {
		return err
	}
	...
	if def {
		if err := osutil.AtomicWriteFile(filepath.Join(t.s.root, defaultsDir, h), []byte(name), 0600); err != nil {
			return err
		}
	} else {
...
func (t *Txn) UpdateLastActivity(ng *NodeGroup) error {
	return osutil.AtomicWriteFile(filepath.Join(t.s.root, activityDir, ng.Name), []byte(time.Now().UTC().Format(time.RFC3339)), 0600)
}
...
func (t *Txn) reset(key string) error {
	...
	return osutil.AtomicWriteFile(filepath.Join(t.s.root, "current"), dt, 0600)
}
