...
func runBake(ctx context.Context, dockerCli command.Cli, targets []string, in bakeOptions, cFlags commonFlags, filesFromEnv bool) (err error) {
	...
	case build.ExecutionModeDeferOutput:
	default:
	...
	if execution.Mode == build.ExecutionModeDeferOutput {
		bh = &build.Handler{
			Execution: execution,
		}
	}
...
func bakeCmd(dockerCli command.Cli, rootOpts *rootOptions) *cobra.Command {
	...
	flags.StringVar(&options.execution, "execution", "fail-fast", `Set target execution behavior ("fail-fast", "defer-output")`)
